...
% mmd2si(x,uniformer(y),M,'ed')*6 == ginimim 2nd row / flavoursi
 ...
 means=zeros(k,M);
 for i=1:k
  means(i,:)=accumarray(ri(:,i),1)./n; % bin count
 end
 if(nargout>1 && d==1)
 % Correlation Ratio estimator
  ...
  Si=zeros(1,k);
  for i=1:k
   Si(i)=means(i,:)*(accumarray(ri(:,i),y,[],@mean)-Ey).^2;
  end
 ...
 % Ki=(n*Ki-M)/(n-M);
  ...
  if(d==1)
   for i=1:k
      mm = accumarray(ri(:,i),y,[],@(z)Ekernquick(y,z,kern));
      seps(i,:)=mm;
      Ki(i)=means(i,:)*mm;
   end
  else
   ...
   for i=1:k
    ...
    mm = accumarray(ri(:,i),ramp,[], @(z)Ekernquick(y,y(z,:),kern));
    seps(i,:)=mm;
    Ki(i)=means(i,:)*mm;
   end
  end
